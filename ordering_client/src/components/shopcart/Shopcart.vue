<template>
  <div class="shopcart" :class="{'highligh':totalCount>0}">
    <div class="shopcart-wrapper">
      ...
      <div class="content-left">
        ...
        <div class="desc-wrapper">
          ...
          <p class="tip" :class="{'highligh':totalCount>0}">shopping car{{poiInfo.shipping_fee_tip}}</p>
        </div>
      </div>
      <!--底部右侧-->
    ...
    <van-dialog v-model="show" :title="'confirm payment' + (totalCount + 9) + '$'" show-cancel-button @confirm="submitPayment" :confirm-button-text="'pay'" :cancel-button-text="'cancel'">
      <!-- <van-field v-model="patient" label="patient" /> -->
      <van-cell center title="isPatient">
  <template #right-icon>
    <van-switch v-model="patient" size="24" />
  </template>
</van-cell>
</van-dialog>
  </div>
    ...
    methods: {
      submitPayment(){
        var _id = this.orderId
         axios.post('http://localhost:6166/payment/add',{
           _id,
           patient:this.patient,
           selectFoods:this.selectFoods.map((data)=>{
            //  data['__id'] = data['_id'];
            //  data['_id'] = undefined;
             return data; 
           }),
      ...
      updatePayment(){
        debugger
        axios.post('http://localhost:6166/payment/update',{
