...
router.post('/reg', (req, res) => {
    let { us, ps, code,number } = req.body
    ...
    .then((data) => {
        if (data.length === 0) {
            ...
            return User.insertMany({ us, ps, time,number })
        } else {
            User.update({_id:data[0]._id},{ps,number}).then((data)=>{
...
router.post('/add', (req, res) => {
    let { us, ps, age, state, sex,number } = req.body
    ...
    .then((id) => {
        const time = (new Date()).getTime()
        return User.insertMany({ us, ps, time, id, age, state, sex ,number})
    })
    .then(() => {
        res.send({ code: 200, msg: '创建成功' })
    })
...
router.post('/updateRole', (req, res) => {
    ...
    if (!_id || !roleId) return res.send({code: 500, msg: '缺少参数'})
    User.update({ _id:_id }, { roleId })
      .then(() => {
        res.send({ code: 200, msg: '修改成功' })
      })
      .catch(() => {
    ...
    .then((data) => {
        if (data.length>0) {
            ...
            Role.find({
              ...
              if (data.length>0) {
                res.send({ code: 200, msg: '登录成功', token ,roleName:data[0].roleName})
              } else{
