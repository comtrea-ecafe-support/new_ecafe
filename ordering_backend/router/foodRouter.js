...
router.post('/add', (req, res) => {
  const { name, price, desc, typeid, img, typename } = req.body
  Food.find({ name })
    .then((data) => {
      if (data.length === 0) {
        return Food.insertMany({ name, price, desc, typeid, img, typename })
      } else {
...
router.post('/update', (req, res) => {
  const { _id, name, price, desc, typeid, img, typename } = req.body
  Food.update({ _id }, { name, price, desc, typeid, img, typename })
    .then(() => {
    ...
    Food.find(query)
      ...
      .then((data) => {
        ...
        data.map(item => {
          temp.push({
            ...item._doc,
            created: item.created ? formatDateTime(new Date(item.created)) : '-',
            updated: item.created ? formatDateTime(new Date(item.updated)) : '-'
          })
        })
        res.send({
          code: 200,
