  ...
  <Modal v-model="open" :title="title">
    <Form ref="formItem" :model="formItem" :label-width="80" :rules="ruleValidate">
      ...
      <FormItem label="price" prop="price">
        <Input v-model="formItem.price" placeholder="Please enter the price" clearable />
      </FormItem>
      <FormItem label="describe" prop="desc">
        <Input v-model="formItem.desc" placeholder="Please enter a description" clearable />
      </FormItem>
      <FormItem label="Classification" prop="typeid">
        <Select v-model="formItem.typeid" clearable placeholder="please choose">
          <Option v-for="item in typeList" :key="item.typeid" :value="item.typeid">
            {{
            item.name
            }}
          </Option>
      ...
      <!-- <FormItem label="upload image" prop="img">
        <Upload ref="addUpload" :before-upload="handleUpload" action>
          <Button v-show="!imgUrl" icon="md-cloud-upload">upload image</Button>
...
import { goodTypePage } from "@/api/goodType";
export default {
  name: "Goods",
  data () {
    return {
